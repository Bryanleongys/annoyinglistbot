...
# /start function
PROMPT_DATE, PROMPT_TIME, END_ADD_TASK, END_DELETE_TASK, RETURN_INITIAL= range(5)
def start(update, context):
    chat_id=update.message.chat.id
    global username
    username=update.message.from_user.username

    text = "Welcome to the most annoying bot of your life, @" + username + "."
    context.bot.send_message(
        chat_id=chat_id,
        text=text
    )
    keyboard_buttons(update, context)
...
def prompt_event(update, context):
    ...
    context.bot.edit_message_text(
        ...
        text = 'What task have you not done?'
    )
    return PROMPT_DATE
...
def prompt_date(update, context):
    ...
    context.bot.send_message(
        chat_id=chat_id,
        text='When will you procrastinate this task until? (YYMMDD e.g. 210101)'
    )
...
def prompt_time(update, context):
    ...
    context.bot.send_message(
        chat_id=chat_id,
        text="What time of the day do you need to complete this by? (HHMM e.g. 0930)"
    )
...
def end_add_task(update, context):
    ...
    if not (user_input.isdigit() and len(user_input) == 4):
        context.bot.edit_message_text(
            chat_id=chat_id,
            text="What are you doing dummy? Input in this format: HHMM"
        )
    ...
    connection = MySQLdb.connect (host = "localhost", user = "root", passwd = "", db = "todolist")
...
def prompt_task_id(update, context):
    ...
    text += "Enter the Task No you want to delete you lazy ass."
